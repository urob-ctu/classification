...
def cross_validate_knn(classifier: KNNClassifier, X: np.ndarray, y: np.ndarray,
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            ...
            accuracies.append(accuracy_score(y_pred, y_val, normalize=True))
            precisions.append(precision_score(y_pred, y_val, average=None))
            recalls.append(recall_score(y_pred, y_val, average=None))
            f1s.append(f1_score(y_pred, y_val, average=None))
