...
class LinearClassifier:
    def __init__(self, num_features: int, num_classes: int, learning_rate: float = 1e-3,
                 weight_scale: float = 1e-4, batch_size: int = 100, reg: float = 1e-3,
    ...
    def train(self, X: np.ndarray, y: np.ndarray) -> list:
        ...
        for i in range(self.num_iters):
            ...
            loss = scores.cross_entropy_loss(y_batch)
            loss += self.W.regularization_loss(1e-3)
            loss += self.b.regularization_loss(1e-3)

            loss.backward()
            loss.step(self.learning_rate)
            loss.zero_grad()
