...
def test_assignment_5_1(
    src_dir: str, verification_file: str, seed: int = 69, generate: bool = False
) -> dict:
    # Dynamically import the module
    engine_module = load_module("utils", "engine")
    mlp_module = load_module(src_dir, "mlp_classifier")
    Tensor = engine_module.Tensor
    MLPClassifier = mlp_module.MLPClassifier
    ...
    if generate:
        np.savez(
            verification_file,
            out=out.data,
            dW1=mlp.params["W1"].grad,
            db1=mlp.params["b1"].grad,
        )
        ...
        try:
            if (
                np.allclose(out.data, expected_out)
                and np.allclose(mlp.params["W1"].grad, expected_dW1)
                and np.allclose(mlp.params["b1"].grad, expected_db1)
            ):
...
def test_assignment_5_2(
    ...
    if generate:
        np.savez(
            verification_file,
            out=out.data,
            dW2=mlp.params["W2"].grad,
            db2=mlp.params["b2"].grad,
        )
        print(f"Successfully generated '{verification_file}'!")
    else:
        expected_out = np.load(verification_file)["out"]
        ...
        try:
            if (
                np.allclose(out.data, expected_out)
                and np.allclose(mlp.params["W2"].grad, expected_dW2)
                and np.allclose(mlp.params["b2"].grad, expected_db2)
            ):
                ret["message"] = f"PASSED!"
                ret["points"] = ret["max_points"]
            else:
...
def test_assignment_5_3(
    ...
    if generate:
        np.savez(
            verification_file,
            out=out.data,
            dW3=mlp.params["W3"].grad,
            db3=mlp.params["b3"].grad,
        )
        print(f"Successfully generated '{verification_file}'!")
    else:
        expected_out = np.load(verification_file)["out"]
        ...
        try:
            if (
                np.allclose(out.data, expected_out)
                and np.allclose(mlp.params["W3"].grad, expected_dW3)
                and np.allclose(mlp.params["b3"].grad, expected_db3)
            ):
                ret["message"] = f"PASSED!"
                ret["points"] = ret["max_points"]
            else:
...
def test_assignment_5_7(
    ...
    try:
        mlp = MLPClassifier(
            num_features, hidden_dim_1, hidden_dim_2, num_classes, num_iters=1
        )
    ...
    if generate:
        np.savez(
            verification_file,
            W1=mlp.params["W1"].data,
            b1=mlp.params["b1"].data,
            W2=mlp.params["W2"].data,
            b2=mlp.params["b2"].data,
            W3=mlp.params["W3"].data,
            b3=mlp.params["b3"].data,
        )
        print(f"Successfully generated '{verification_file}'!")
        ...
        try:
            for name in mlp.params.keys():
                ...
                if not np.allclose(
                    mlp.params[name].grad, np.zeros_like(mlp.params[name].grad)
                ):
                    ret["message"] = (
