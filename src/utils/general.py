...
def normalize_torch(*arrays: torch.Tensor) -> List[torch.Tensor]:
    ...
    return {"normalized_arrays": [(array - mean) / std for array in arrays], "mean": mean, "std": std}


def dataset_stats(X_train: Union[np.ndarray, torch.Tensor], y_train: Union[np.ndarray, torch.Tensor], 
                  X_val: Union[np.ndarray, torch.Tensor], y_val: Union[np.ndarray, torch.Tensor], 
                  X_test: Union[np.ndarray, torch.Tensor], y_test: Union[np.ndarray, torch.Tensor], 
    ...
    num_classes = len(np.unique(y_train)) if isinstance(y_train, np.ndarray) else len(torch.unique(y_train))
