...
class Visualizer:
    ...
    def __init__(self, train_split: Tuple[Union[np.ndarray, torch.Tensor], 
                                          Union[np.ndarray, torch.Tensor]],
                 val_split: Tuple[Union[np.ndarray, torch.Tensor], 
                                  Union[np.ndarray, torch.Tensor]] 
                                  = (np.array([]), np.array([])),
                 test_split: Tuple[Union[np.ndarray, torch.Tensor], 
                                   Union[np.ndarray, torch.Tensor]] 
                                   = (np.array([]), np.array([]))):
    ...
    #     None
    #     """
    #     x_rng = np.linspace(self.x_lim[0], self.x_lim[1], 50)
    #     y_rng = np.linspace(self.y_lim[0], self.y_lim[1], 50)
    #     z_rng = np.linspace(-5, 5, 50)  # Adjust this range as needed
    #     xx, yy, zz = np.meshgrid(x_rng, y_rng, z_rng)
    ...
    #         values = logits[:, c].reshape(xx.shape) - zz
    ...
    #     y_train = self.y_train if isinstance(self.y_train, np.ndarray) else self.y_train.numpy()
    #     for c in range(self.num_classes):
    #         mask = y_train == c
    #         color = f'rgb{tuple(int(val * 255) for val in self.colors[c])}'
    ...
    def _plot_data(self, features: Union[np.ndarray, torch.Tensor], 
